...
public class AlbumService {
    ...
    public void updateAlbumLikes(int id, int likes) throws ChangeSetPersister.NotFoundException {
        Album album = albumRepository.findById(id).orElseThrow(() -> new ChangeSetPersister.NotFoundException());
        album.setLikes(likes);
        albumRepository.save(album);
    }

    // Get the number of likes for an album
    public int getAlbumLikes(int Id) {
        Album album = albumRepository.findById(Id).orElse(null);
        return album != null ? album.getLikes() : 0;
    }
}